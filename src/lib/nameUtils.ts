...
/**
 * Generates a temporary nickname from the user's name initials. First name initial maps to an
 * adjective, last name initial maps to an animal.
...
export function generateTempNickname(name: string): string {
  ...
  const secondInitial = (
    parts.length > 1 ? parts.at(-1)![0] : parts[0]?.[1] ?? "A"
  ).toUpperCase();
